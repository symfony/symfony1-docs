...
YAML はデータを記述するためのシンプルな言語です。PHP のように、文字列、ブール値、浮動小数点、整数のようなシンプルなデータ型のための構文をもちます。しかし PHP とは異なり、配列 (シーケンス) とハッシュ (マッピング) のあいだに違いがあります。
...
YAML の null 値は `null` もしくはチルダ (`~`) で表現されます。
...
YAML ファイルでインデントを使うときに覚えておくことが1つあります: *インデントには1つもしくは複数のスペースを使い、タブを使ってはなりません*。
...
スコープを表現するためにインデントよりも明確なインジケータを使うことで、コレクションの表現するためにフロースタイルを選ぶことができます。
