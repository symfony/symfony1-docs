...
文字列に改行を入れるとき、パイプ (`|`) によって示されるリテラルスタイルを使うことができます。これは複数行にわたる文字列を表します。リテラルでは、改行は保たれます:
...
シーケンスでは角かっこ (`[]`) で囲まれカンマで区切られるリストとして記述できます:

    [yml]
    [PHP, Perl, Python]

マッピングでは波かっこ (`{}`) で囲まれカンマで区切られるキーもしくは値として記述できます:
