...
`param` 設定をセットすることでトランスポートを細かく調整できます。組み込みのトランスポートクラスと異なるパラメータに関して Swift Mailer の公式ドキュメントの [「Transport Types」](http://swiftmailer.org/docs/transport-types) のセクションで知る必要のあるすべての知識が説明されています。
...
`send_http_headers` オプションはレスポンスのコンテンツに加えて HTTP レスポンスヘッダーを送信するかを指定します。出力の後でヘッダーを送信しようとすると警告を発してくれる PHP の `header()` 関数でヘッダーが送信されるので、この設定はテストの際にもっとも役立ちます。
