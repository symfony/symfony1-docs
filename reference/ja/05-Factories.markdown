...
`relative_url_root` オプションは URL のなかのフロントコントローラより前の部分を定義します。たいていの場合、これはフレームワークによって自動的に検出されるので変更する必要はありません。
...
`true` にセットされている場合、`generate_shortest_url` オプションはルーティングシステムに実現可能な最短ルートを生成するよう指示します。symfony 1.0と 1.1 と後方互換性のあるルートがほしい場合は、`false` にセットします。
