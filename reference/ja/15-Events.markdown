...
`notify()` メソッドはすべてのリスナーに通知します。リスナーは値を返すことはできず、すべてのリスナーが実行されることが保証されています。
...
`filter()` メソッドは通知オブジェクトによって2番目の引数として渡され、2番目の引数としてリスナーの callable によって読み出される任意の値をすべてのリスナーがフィルタリングできることを通知します。すべてのリスナーに値が渡され、これらはフィルタリングされた値を返さなければなりません。すべてのリスナーが実行されることが保証されています。
