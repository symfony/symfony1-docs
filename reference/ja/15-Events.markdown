...
`filter()` メソッドは、通知オブジェクトによって2番目の引数として渡される任意の値をフィルタリングし、結果がリスナーの2番目の引数である callable によって読み出されることを通知します。すべてのリスナーに値が渡され、これらはフィルタリングされた値を返さなければなりません。すべてのリスナーの実行が保証されています。
