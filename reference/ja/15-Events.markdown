...
リクエストが `sf_format` パラメータセットをもつとき `view.configure_format` イベントが通知されます。symfony が設定を変更するもしくはレイアウトの設定を解除するなどの単純な作業を行った後にこのイベントが通知されます。リクエストされるフォーマットにしたがってビューとレスポンスオブジェクトを変更することがこのイベントによって可能になります。
