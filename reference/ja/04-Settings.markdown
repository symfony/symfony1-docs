...
>**TIP**
>`generate:app` タスクでアプリケーションを作成するときに `--escaping-strategy` オプションを指定することでこの設定を自動的にセットできます。
...
`web_debug_web_dir` は Web デバッグツールバーのアセットへの Web パスをセットします (画像、スタイルシートと  JavaScript ファイル)。
