...
プロジェクトで~Subversion~を使う場合、プロジェクトを最新のsymfony1.4のタグにリンクすることもできます:
...
`svn up`を実行するたびに、最新のsymfony 1.4が手に入ります。
...
15日目、`config/error/`ディレクトリで`error.yaml.php`と`exception.yaml.php`ファイルを作ることで`YAML`フォーマット用のエラーページを設定しました。
