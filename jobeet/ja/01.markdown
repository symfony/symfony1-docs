...
このディレクトリを見てみると、Web ~アセット~ (`css/` と `images/`) と2つのフロントコントローラーファイル用のサブディレクトリが見つかります。フロントコントローラーは Web 公開のルートディレクトリの下に必要な唯一の PHP ファイルです。ほかのすべての PHP ファイルはブラウザーから隠すことが可能で、~セキュリティ~に関してよいアイデアです。
