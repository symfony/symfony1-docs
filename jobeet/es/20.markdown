...
Las notificaciones de los Eventos son gestionados por  ([`sfEventDispatcher`](http://www.symfony-project.org/api/1_3/sfEventDispatcher)),  el objeto event dispatcher.
...
Cuando el dispatcher llama al método `methodNotFound()`, este pasa un objeto [`sfEvent`](http://www.symfony-project.org/api/1_3/sfEvent).
