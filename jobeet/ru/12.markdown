...
Заменим содержимое сгенеренного `layout.php` файла следующим кодом:
    ...
    <html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en" lang="en">
      <head>
        ...
        <?php use_stylesheet(Шadmin.cssШ) ?>
      ...
      <body>
        <div id="container">
          <div id="header">
            <h1>
              <a href="<?php echo url_for(Ш@homepageШ) ?>">
          ...
          <div id="menu">
            <ul>
              <li>
<propel>
                <?php echo link_to(ШJobsШ, Ш@jobeet_jobШ) ?>
</propel>
<doctrine>
                <?php echo link_to(ШJobsШ, Ш@jobeet_job_jobШ) ?>
</doctrine>
              </li>
              <li>
<propel>
                <?php echo link_to(ШCategoriesШ, Ш@jobeet_categoryШ) ?>
</propel>
<doctrine>
                <?php echo link_to(ШCategoriesШ, Ш@jobeet_category_categoryШ) ?>
</doctrine>
              </li>
            </ul>
          </div>
...
`actions.class.php` также пусты:
    ...
    require_once dirname(__FILE__).Ш/../lib/jobGeneratorConfiguration.class.phpШ;
    require_once dirname(__FILE__).Ш/../lib/jobGeneratorHelper.class.phpШ;
...
symfony сгенерирует исключение, сообщающее Вам, что необходимо определить метод `executeBatchExtend()`:
    ...
    class jobActions extends autoJobActions
    {
      ...
      {
        $ids = $request->getParameter(ШidsШ);
...
<doctrine>
        ...
        $this->getUser()->setFlash(ШnoticeШ, ШThe selected jobs have been extended successfully.Ш);

<propel>
        $this->redirect(Ш@jobeet_jobШ);
</propel>
<doctrine>
        $this->redirect(Ш@jobeet_job_jobШ);
</doctrine>
      }
    }
...
<propel>
        $this->setExpiresAt(time() + 86400 * sfConfig::get(Шapp_active_daysШ));
</propel>
<doctrine>
        $this->setExpiresAt(date(ШY-m-dШ, time() + 86400 * sfConfig::get(Шapp_active_daysШ)));
</doctrine>
        $this->save();
...
<doctrine>
        $nb = Doctrine::getTable(ШJobeetJobШ)->cleanup(60);
</doctrine>

        if ($nb)
        {
          $this->getUser()->setFlash(ШnoticeШ, sprintf(Ш%d never activated jobs have been deleted successfully.Ш, $nb));
        }
        else
        {
          $this->getUser()->setFlash(ШnoticeШ, ШNo job to delete.Ш);
        }
...
из файла `lib/model/doctrine/JobeetJobTable.class.php`.
    ...
    class JobeetJobTable extends Doctrine_Table
    {
      public function retrieveBackendJobList(Doctrine_Query $q)
      {
        ...
        $q->leftJoin($rootAlias . Ш.JobeetCategory cШ);
        return $q;
      }
...
<doctrine>
    ...
    class JobeetCategoryForm extends BaseJobeetCategoryForm
    {
      ...
      {
<propel>
        unset($this[Шjobeet_category_affiliate_listШ]);
</propel>
<doctrine>
        unset($this[Шcreated_atШ], $this[Шupdated_atШ], $this[Шjobeet_affiliates_listШ]);
</doctrine>
      }
    }
...
<doctrine>
    ...
    class JobeetJobForm extends BaseJobeetJobForm
    {
      public function configure()
      {
        ...
        $this->validatorSchema[ШemailШ] = new sfValidatorAnd(array(
          $this->validatorSchema[ШemailШ],
      ...
      {
        unset(
          $this[Шcreated_atШ], $this[Шupdated_atШ],
          $this[Шexpires_atШ], $this[Шis_activatedШ],
          $this[ШtokenШ]
        );
      }
    }
...
<doctrine>
    ...
    class BackendJobeetJobForm extends JobeetJobForm
    {
      public function configure()
      {
        ...
        $this->widgetSchema[ШlogoШ] = new sfWidgetFormInputFileEditable(array(
          ШlabelШ     => ШCompany logoШ,
          Шfile_srcШ  => Ш/uploads/jobs/Ш.$this->getObject()->getLogo(),
          Шis_imageШ  => true,
          Шedit_modeШ => !$this->isNew(),
          ШtemplateШ  => Ш<div>%file%<br />%input%<br />%delete% %delete_label%</div>Ш,
        ));

        $this->validatorSchema[Шlogo_deleteШ] = new sfValidatorPass();
      }
