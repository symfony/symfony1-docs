...
В этой книге Вы никогда не увидите подобные изречения и мы будет писать
...
ярлык `symfony` в корневом каталоге проекта для 
...
Ещё раз - задача `generate:app` создаёт стандартную
...
Данная конфигурация позволяет слушать порт `8080` на вашей машне, так что
...
(see the next section for more information about environments).
