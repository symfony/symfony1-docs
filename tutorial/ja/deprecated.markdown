...
次のメソッドと関数は symfony 1.3 かそれ以前で廃止予定になり、symfony 1.4 で削除されます:
  ...
  * `sfContext::retrieveObjects()`: このメソッドを使うのは `ObjectHelper` のみで、廃止予定です
...
次のクラスは symfony 1.3 で削除されます:
  ...
  * `sfCommonFilter`: 結果とコードをマイグレートする方法に関する情報は "プロジェクトを1.2から1.3にアップグレードする"のチュートリアルの"共通フィルターの削除"を参照してください。
...
PHP のインクルードパスからヘルパーをロードする機能は1.3 で廃止予定になり1.4で削除されました。ヘルパーはプロジェクト、アプリケーションもしくはモジュールの `lib/helper/` ディレクトリのどれか1つに設置しなければなりません。
...
次の設定 (`settings.yml` 設定で管理される)は symfony 1.3 から削除されました:
  ...
  * `sf_lazy_cache_key`: symfony 1.2.6 で大きなパフォーマンス改善として導入され、この設定はビューキャッシュのために遅延キャッシュキージェネレーションを有効にすることを許可しました。コア開発者は遅延がベストなアイデアと考える一方で、中にはアクション自身がキャッシュ可能ではないときでも呼び出される `sfViewCacheManager::isCacheable()` に頼る人もいました。symfony 1.3 に関しては、ふるまいは `sf_lazy_cache_key` が `true` にセットされる場合と同じになります。
...
次のタスクは symfony 1.3 で廃止予定で、symfony 1.4 で削除されます:
  ...
  * `propel:init-admin`: このタスクは symfony 1.0用の admin ジェネレーターモジュールを生成しました。
