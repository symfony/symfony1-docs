 ...
 * `sfToolkit::removeArrayValueForPath()`、
  ...
  * `sfTestFunctionalBase` の次のメソッド: `isRedirected()`、`isStatusCode()`、`responseContains()`、
    ...
    `isUserCulture()`、`isRequestFormat()` と `checkResponseElement()`: これらのメソッドは symfony 1.2 以降で廃止予定になり、テスタークラスに置き換わります。

  * `sfTestFunctional` の次のメソッド: `isCached()`、 `isUriCached()`: これらのメソッドは symfony 1.2 以降で廃止予定になり、テスタークラスに置き換わります。
...
次のクラスは symfony 1.3 で廃止予定になり symfony 1.4 で削除されます:
  ...
  * `sfDoctrineDataRetriever`、`sfPropelDataRetriever`: これらのクラスを使っていたのは `ObjectHelper` だけなので廃止予定です。
...
PHP のインクルードパスからヘルパーをロードする機能は symfony 1.3 で廃止予定になり symfony 1.4 で削除されます。ヘルパーの設置場所はプロジェクト、アプリケーションもしくはモジュールの `lib/helper/` ディレクトリのどれか1つにしなければなりません。
